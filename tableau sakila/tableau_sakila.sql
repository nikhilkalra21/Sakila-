...
-- 1.1  Rank total rental amount (payment amount) by country – tabular report & sql query
SELECT cy.country AS customer_country, SUM(p.amount) AS total_rental_sales, DENSE_RANK() OVER( ORDER BY SUM(p.amount) DESC) AS SalesRank
...
-- 1.2 Display total rental amounts (payment amount) by city
SELECT c.city AS customer_city, sum(p.amount) AS total_rental_sales
FROM payment p INNER JOIN
rental r 
...
-- 1.3 Contribution of Countries & Cities (in hierarchy) by rental amount 
...
-- 1.4 Rental amounts by countries for PG & PG-13 rated films
...
-- 1.5 Top 20 cities by number of customers who rented
...
-- 1.6 Top 20 cities by number of films rented 
